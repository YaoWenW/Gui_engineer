...
class MatrixCalculator(QMainWindow):
    ...
    def initUI(self):
        ...
        self.info = [
                    ...
                    '纵缝螺栓为螺栓连接取[0, 20)，\n双斜平行M30/双斜平行M36时取[20, 40)，\n双斜平行M39/三斜平行M30/三斜平行M36/四斜平行M30取[40, 60)，\n三斜交叉M30/三斜交叉M36/四斜交叉M30取[60, 80)，\n三斜平行M39/四斜平行M36/四斜平行M39取[80, 100)',
                    '环缝螺栓数量取值根据螺栓的实际数量取值。',
                    ...
                    '取管片最大上浮量（单位为mm）',
                    '取相邻管片径向错台或者环向错台中的较大值（单位为mm）',
                    '取轴线偏差值（单位为mm）',
                    '取衬砌环内错台（mm）、衬砌环内错台（mm）、管片椭圆度（ % 。）中的较大值。',
        ...
        Y = [90, 90, 70, 8, 2.827, 70, 50, 30, 58, 70, 0.566, 2.5, 6, 8.68, 45, 3.1, 7, 4.5, 1.5, 21]
